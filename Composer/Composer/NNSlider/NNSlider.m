...
- (UIBezierPath *)linePath
{
    ...
    if (self.isVertical == NO) {
        ...
        float endX = self.bounds.size.width;
        [path moveToPoint: CGPointMake(originX, originY)];
        [path addCurveToPoint: CGPointMake(0, originY) controlPoint1: CGPointMake(0, originY) controlPoint2: CGPointMake(0, originY)];
        [path addCurveToPoint: CGPointMake(endX, originY) controlPoint1: CGPointMake(0, originY) controlPoint2: CGPointMake(0, originY)];
        [path stroke];
    } else {
...
- (void)updateValueTo:(float)value {
    self.value = (value*self.valueScale)+self.minValue;
...
- (void)setValueTo:(float)value {
    ...
    if (self.isVertical) {
        if (self.isSegmented) {
            if (self.isInt) {
                y = (self.frame.size.height / self.segments) * (self.value-self.minValue);
            } else {
                y = (self.value-self.minValue) / self.frame.size.height;
            }
        }
        
        point = CGPointMake(self.center.x, y);
        knob = CGPointMake(knobLayer.position.x, (((y*self.bounds.size.height)-self.bounds.size.height)+knobCenterOffset));
        
    } else {
        if (self.isSegmented) {
            if (self.isInt) {
                x = (self.frame.size.width / self.segments) * (self.value-self.minValue);
            } else {
                x = (self.value-self.minValue) / self.frame.size.width;
            }
        }
        
        point = CGPointMake(x, self.center.y);
        knob = CGPointMake(x-knobCenterOffset, knobLayer.position.y);
    }
    
    [self animateKnob:knobLayer to:knob];
    [self animateLabelTo:point];
}
